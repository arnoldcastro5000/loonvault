...
**Mitigating controls:** App DB users use RDS IAM auth, no stored DB credential, so there is no secret value to leak or place in state (closes the original T-032 vector by construction); the RDS master password is RDS-managed (`manage_master_user_password`), never referenced by value in Terraform. Each `rds-db:connect` grant is scoped to a single dbuser ARN; OPA/CloudTrail detection covers IAM policy widening (T-033). KMS: CloudTrail KMS data events (Decrypt/GenerateDataKey); automatic key rotation annually; CMK deletion requires 7–30 day pending period; EventBridge rule fires on `DisableKey`, `ScheduleKeyDeletion`, `CancelKeyDeletion` (detection rule 7, G-04). Anomalous `GetSecretValue` on the RDS master secret triggers an EventBridge alert (detection rule 8, G-12): no automated, always-on principal holds this permission, so every event resolves to either RDS-managed rotation, a named operator maintenance action (db-init/cloud-exit), or a compromise (expected-but-logged; see runbook triage). RDS IAM token validation routes through the IAM/STS control plane (us-east-1), mitigated by the regional STS endpoint; warm DB connections survive a short control-plane blip since tokens are only needed at connect time (T-035).

**Gap: T-034 -- CLOSED (G-04).** EventBridge rule 7 fires on `DisableKey`, `ScheduleKeyDeletion`, and `CancelKeyDeletion` (`source: aws.kms`, `eventSource: kms.amazonaws.com`) → SNS (`infra/loonvault/detection.tf`).
...
**Mitigating controls:** EventBridge rule fires on `StopLogging`/`DeleteTrail`/`UpdateTrail` (1 occurrence → SNS); CloudTrail log bucket has deletion-deny policy (`s3:DeleteObject` denied, including root); CloudTrail log file validation detects tampering; CloudTrail management events capture detection-infrastructure changes; nine detection rules — five EventBridge rules plus four CIS metric-filter alarms (ADR-0014) — cover the highest-signal events across all six B-13 Cyber Security principles; SNS alerts have confirmed subscriber (email); rule 9 fires on suppression of either transport: `DeleteRule`/`DisableRule`/`RemoveTargets` (EventBridge) and `DeleteMetricFilter`/`DeleteAlarms`/`DisableAlarmActions` (metric filters/alarms) (G-05).
...
**Gap: T-038 -- CLOSED (G-05).** EventBridge rule 9 fires on suppression-class operations against **both detection transports**: `DeleteRule`, `DisableRule`, `RemoveTargets` (`eventSource: events.amazonaws.com`) and — since rules 2/3/4 moved to CIS metric-filter alarms (ADR-0014) — `DeleteMetricFilter` (`logs.amazonaws.com`) and `DeleteAlarms`/`DisableAlarmActions` (`monitoring.amazonaws.com`) → SNS (`infra/loonvault/detection.tf`). `PutRule`/`PutMetricFilter` are deliberately excluded: every `loonvault-apply` fires them, making them too noisy for a single-developer workflow. Residual limitation: rule 9 cannot detect its own deletion (self-referential detection problem). Production fix: cross-account CloudTrail delivery to a dedicated monitoring account whose rules cannot be modified by a compromised workloads-account principal.
...
**Mitigating controls (T-049):** The published detection thresholds are a known and accepted tradeoff of the public posture. Eight of the nine detection rules are single-occurrence for high-signal events (root usage, CloudTrail disabled, IAM policy changed, SG changed, no-MFA sign-in, KMS key lifecycle, anomalous GetSecretValue, detection rule tampering): these cannot be evaded by staying below a threshold — the three that run as CIS metric-filter alarms (rules 2/3/4, ADR-0014) alarm at ≥ 1 event in 5 minutes, which any single event trips. Only the AccessDenied spike (>5 in 5 min) is threshold-based. Mitigation: consider lowering the threshold post-deploy or adding a secondary metric for slower enumeration (e.g., >20 AccessDenied in 30 min). The detection rule configuration intentionally errs toward false positives over false negatives for high-signal events.
...
| G-04 | No detection rule for KMS CMK lifecycle events | T-034 | Medium | **Closed** | EventBridge rule 7: `DisableKey`, `ScheduleKeyDeletion`, `CancelKeyDeletion` (`source: aws.kms`) → SNS (`infra/loonvault/detection.tf`) |
| G-05 | No detection rule for EventBridge rule suppression | T-038 | Medium | **Closed** | EventBridge rule 9: `DeleteRule`, `DisableRule`, `RemoveTargets` (`source: aws.events`) → SNS. `PutRule` excluded (Terraform apply false positives). Residual: self-referential detection gap documented in T-038 (`infra/loonvault/detection.tf`) |
...
| G-12 | No detection rule for anomalous `GetSecretValue` against the RDS master secret | T-034 | Medium | **Closed** | EventBridge rule 8: `GetSecretValue` (`source: aws.secretsmanager`) → SNS. Zero-standing-baseline signal: no automated, always-on principal holds this permission, so every event is RDS-managed rotation, a named operator maintenance action (db-init/cloud-exit), or a compromise. Expected-but-logged; legitimate callers and triage in runbook (`infra/loonvault/detection.tf`) |
