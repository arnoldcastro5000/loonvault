...
| G-12 | No detection rule for anomalous `GetSecretValue` against the RDS master secret | T-034 | Medium | **Closed** | EventBridge rule 8: `GetSecretValue` (`source: aws.secretsmanager`) → SNS. Zero-baseline signal: no Lambda role holds this permission; any call is a compromise indicator. Known false positive: RDS-managed rotation (see runbook) (`infra/loonvault/detection.tf`) |
