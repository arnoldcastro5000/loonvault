...
resource "aws_s3_bucket" "raw" {
  ...
  # Ephemeral backend: raw data is reproducible (ingest re-pulls from BoC Valet),
  # so allow `terraform destroy` to delete the bucket even when it holds objects.
  force_destroy = true
}
...
# ── S3 snapshots bucket ───────────────────────────────────────────────────────
# MOVED to the always-on ../frontend stack (it must survive a backend destroy —
# ADR-0004). The Transform Lambda still writes to it by name; see
# local.snapshots_bucket in main.tf and the WriteSnapshots statement in iam.tf.
